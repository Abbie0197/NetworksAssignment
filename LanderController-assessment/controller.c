...
void *dashboard(void *data)
{
    ...
    while (true) {
        ...
        // TODO: Task XXX, construct buffer array with the sprintf methond with the following format DONE
        // "fuel:%f\naltitude:%f\n", landercond.fuel, landercond.altitude
	sprintf(buffer, "fuel:%f\naltitude:%f\n", landercond.fuel, landercond.altitude);

        

        // TODO: Task XXX, send the buffer to the dashboard by using sendto method DONE
	ssize_t sendto(int d, const void *buffer, size_t len, int flags, const struct sockaddr *daddr, socklen_t bufsize);
...
int main(int argc, char *argv[])
{
    ...
    //TODO: Task XXX create one thread for dashboard with the function pthread_create DONE
